...
**Sales priority:** Low — for core CPMS displacement. The platform is proprietary, in-house, backed by Eni's balance sheet, and the company has demonstrated willingness to absorb multi-year operational debt rather than switch. No realistic CPMS platform sale opportunity exists at the main entity level in the near term. At main entity (Eni Plenitude / Plenitude On The Road S.r.l.) level: No forced platform decision. At ACEA Innovation subsidiary level: Yes — B.O.M.T.S. platform acquired April 2026 faces a near-term integration or replacement decision within 6–12 months.
Trigger: Eni Plenitude announces an IPO process or strategic sale mandate for the e-mobility business; or Ares Management discloses dissatisfaction with operational performance in investor communications or mandates a CEO change. Either event makes CPMS scalability a near-term investor question.

**Entry point if pursued:** ACEA Innovation's B.O.M.T.S. CPO platform (acquired April 2026 via ACEA Energia acquisition). This is a separate technology decision from the main Plenitude in-house platform — a recently acquired CPO stack that needs integration or replacement. The integration timeline and architecture decision for B.O.M.T.S. is likely being made now (H1/H2 2026). Accessibility qualifier: the opening exists only if Plenitude's in-house CPMS lacks a capability ACEA's network requires, or if there is a technical barrier to direct migration. The more likely internal path is migration of ACEA's ~1,000 CPs onto Plenitude's own platform at no incremental licence cost — which would close the opening without any external vendor involvement. Verify the integration architecture decision before pursuing. Fleet DLM is a second angle: the current in-house platform has no depot management capability, and fleet is explicitly a growth segment with named enterprise clients (LeasePlan, BMW). A modular fleet/DLM module that plugs alongside the existing CPMS rather than replacing it is the more realistic entry.

**Adjacent leads:** ACEA Innovation (the B.O.M.T.S. platform entity, now an Eni Plenitude subsidiary) — separate CPO technology procurement decision, near-term timing window (integration architecture decision likely within 6 months of April 2026 acquisition). Artelia (external general contractor for DACH station construction) — not a software lead but confirms Plenitude is using external partners for DACH expansion, indicating the supply chain for that geography is not captive.
...
The near-term opportunity is not CPMS displacement but the **ACEA Innovation B.O.M.T.S. decision**. This is a separately developed CPO platform that Plenitude has just acquired. The integration decision will be made by people who did not build the Plenitude in-house CPMS — making it genuinely open to external vendor evaluation in a way the core Plenitude platform is not. The timing window is narrow: post-acquisition architecture decisions typically resolve within 6–12 months of close. Fleet DLM is a second-order angle — if Plenitude's fleet book grows as intended, the current RFID-only product will hit a ceiling with enterprise clients, creating demand for a DLM or depot management module that could sit alongside the in-house CPMS rather than replacing it.
